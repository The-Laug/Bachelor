...
// Function to check if a given domain exists in a specified file
let domainExists (domain :String)  (filePath :String)   =
    File.ReadLines(filePath)
    |> Seq.exists (fun line -> line.Contains(domain))

// Module to maintain a global counter
...
// Function to add a simple sum domain to a specified file if it does not already exist
...
// Function to add a square sum domain to a specified file if it does not already exist
...
axiom rightRecursionSquareSum {
    forall i:Int, N: Int :: 
        0 <= i < N ==> squaresum(i,N-1) + N*N
}

axiom leftRecursionSquareSum {
    forall i:Int, N: Int :: 
        0 <= i < N ==> i*i + squaresum(i+1,N)
}
}
")

// Function to add a setified sum domain to a specified file if it does not already exist
...
// Function to add a generic sum domain to a specified file if it does not already exist
...
axiom rightRecursionGenericSum" + (count) + " {
    forall i:Int, N: Int :: 
        0 <= i < N ==> genericSum" + (count) + "(i,N-1) + " + interpretInnerFunction(innerFunction,indexVariable,"N") + "
}

axiom leftRecursionGenericSum" + (count) + " {
    forall i:Int, N: Int :: 
        0 <= i < N ==> genericSum" + (count) + "(i+1,N) + " + interpretInnerFunction(innerFunction,indexVariable,"i") + "
}
...
// Function to interpret a sum expression and transform it into the corresponding domain and function calls
...
// Function to process a line by searching for sum expressions and transforming them
...
// Function to write processed strings to a file and include the summation domain import
...
// Main function to handle file processing based on command line arguments
